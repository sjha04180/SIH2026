...
import { Printer, Award, CheckSquare, Info, Globe, Code } from 'lucide-react';
...
export default function ReportClient({ student, sessionName }: ReportClientProps) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto font-sans p-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start space-x-3 print:hidden shadow-sm">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center text-xs">
            <div className="bg-slate-55 border border-slate-200 p-3 rounded-lg">
              <span className="text-[9px] uppercase font-bold text-slate-500">Total Activities</span>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2 text-xs">
              {profileLinks.length === 0 ? (
                <li className="text-slate-400 italic">No links uploaded.</li>
              ) : (
                profileLinks.map((link: any) => (
                  <li key={link.id} className="flex justify-between items-center bg-slate-50/50 p-2 border border-slate-100 rounded-lg">
                    ...
                    <span className="font-mono text-slate-655 truncate max-w-[200px]">{link.profileUrl}</span>
                  </li>
                ))
              )}
            </ul>
              ...
              <div key={idx} className={`pt-4 ${idx === 0 ? 'pt-0' : ''} text-xs space-y-2 print:break-inside-avoid`}>
                <div className="flex justify-between items-start gap-3">
                  ...
                  <span className={`inline-flex px-2 py-0.5 rounded text-[9px] font-bold uppercase border ${
                    ...
                    item.status === 'SELF_DECLARED' ? 'bg-blue-50 border-blue-150 text-blue-700' : 'bg-slate-50 border-slate-200 text-slate-655'
                  }`}>
